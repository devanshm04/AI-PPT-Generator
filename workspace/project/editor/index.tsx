...
// {DESIGN_STYLE}. responsive design 16:9 layout for slides.
...
// //     console.log("Generated Slide HTML:", html);
...
//         // got the below code from firebase gemini live model docs
...
import { useParams } from 'react-router-dom'
import type { Project } from '../outline';
import SliderFrame from './../../../src/components/custom/SliderFrame';
...
function Editor() {
    ...
    const SaveAllSlides = async () => {
        // Filter out undefined slides before saving
        const validSlides = sliders?.filter((slide: any) => slide && slide.code);
        
        if (!validSlides || validSlides.length === 0) {
            console.warn("⚠️ No valid slides to save");
            return;
        }
